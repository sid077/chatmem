...
30. **`chatmem install` runs `codesign --force --sign - <bin>` (ad-hoc) on macOS before launchd loads the plist.** Prevents launchd's Gatekeeper prompt on every daemon start. Ad-hoc is a local-machine-only trust anchor — real Developer-ID + notarization (workflow-side, gated on `MACOS_CERT_P12` secret) is what removes the prompts for *other* users. `codesignStatus()` in `cmd/chatmem/service.go` distinguishes ad-hoc / Developer-ID / notarized and is surfaced in both `chatmem status` and `chatmem doctor`.
31. **Release signing is entirely opt-in via GitHub secrets.** `.github/workflows/release.yml` guards RPM signing on `GPG_PRIVATE_KEY` and macOS notarization on `MACOS_CERT_P12` + `APP_STORE_CONNECT_KEY`. When secrets are missing, the workflow behaves exactly as it did in v0.3.1. When present, it signs + regenerates checksums + overwrites the release assets + publishes the public GPG key to gh-pages + flips the served `.repo` file to `gpgcheck=1`. See `docs/release-signing-setup.md` for user-side secret setup.
